...
void Bullets::buildBullet(int type, int a_signature, Transform t)
{
	...
	case 2:
		shot = PUNCH_SHOT;
		collider.hull.vsize = 4;
		collider.hull.vertices[0] = { 0,1 };
		collider.hull.vertices[1] = { 2,-1 };
		collider.hull.vertices[2] = { 0,-2 };
		collider.hull.vertices[3] = { -2,-1 };
		bulletColor = GREEN;
		speed = 300;
		timer = 3.f;
		rigidbody.drag = 2;
		rigidbody.mass = 5;
		rigidbody.addImpulse(transform.getUp() * speed * 5);
		break;
	case 3:
		shot = BURST_CANNON;
		collider.hull.vsize = 4;
		collider.hull.vertices[0] = { 0,1 };
		collider.hull.vertices[1] = { 1,-.8f };
		collider.hull.vertices[2] = { 0,-2 };
		collider.hull.vertices[3] = { -1,-.8f };
		bulletColor = BLUE;
		speed = 300;
		timer = 3.f;
		rigidbody.drag = -3;
